rsdata <- rsdata %>%
  mutate(
    ...
    avqi_ef40 = if_else(!is.na(shf_ef_cat) & shf_ef_cat == "<40%", 1, 0),
    qi3_1bbl = case_when(
    ...
    avqi_loop = if_else(!is.na(shf_xray) &
      shf_xray %in% c("Pulmonary congestion", "Pulmonary congestion & cardiomegaly"), 1, 0),
    qi3_4loop = case_when(
...
qivarsmeta <- qivarsmeta %>%
  mutate(
    qishortname = case_when(
      ...
      qivar == "sens_qi2_4lab" ~ "Blood tests documented (sensitivity)",
    ...
    qilongname = case_when(
      ...
      qivar == "sens_qi2_4lab" ~ "Main (2.4): Blood tests documented (sensitivity)",
    ...
    noadjvars = case_when(
      ...
      "shf_ef, shf_ntprobnp, shf_anemia, shf_gfrckdepi, shf_location",
      qivar %in% c("comp_qi_opbased2", "comp_qi_opbased2_cat") ~
      "shf_ef, shf_ntprobnp, shf_anemia, shf_gfrckdepi, shf_durationhf, shf_location",
      TRUE ~ NA_character_
    ),
    qivartype = case_when(
