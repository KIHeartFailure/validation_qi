...
xvars <- c("shf_location", "shf_sex", "shf_age_cat")
...
orfunc <- function(qi) {
  ...
  for (i in seq_along(xvars)) {
    ...
    if (checkav == 4) {
      mod <- glm(formula(paste0(qi, " == 'Yes' ~ ", xvars[i])),
        data = rsdata,
      ...
      out[(i * 2):(i * 2 + 1)] <- c(
        "ref",
        paste0(
          fn(exp(smod$coefficients[2, "Estimate"]), 2),
          " (",
          fn(exp(smod$coefficients[2, "Estimate"] - global_z05 * smod$coefficients[2, "Std. Error"]), 2),
          "-",
          fn(exp(smod$coefficients[2, "Estimate"] + global_z05 * smod$coefficients[2, "Std. Error"]), 2),
          "), ",
          fn(smod$coefficients[2, "Pr(>|z|)"], 3, p = T)
        )
      )
    }
  }
...
default_kable(qior,
  ...
  add_header_above(c(" " = 1, "Location" = 2, "Sex" = 2, "Age" = 2))
```
