...
survfunc <- function(qi, times, events) {
  out <- data.frame(matrix(NA, ncol = 7, nrow = 1))
  colnames(out) <- c("QI", rep(c("Incidence No/Yes", "Crude HR (95% CI), p-value", "Adjusted HR (95% CI), p-value"), 2))
...
survfunc2 <- function(times2, events2) {
  for (j in seq_along(qivarsuse)) {
    tmp <- survfunc(qi = qivarsuse[j], times = times2, events = events2)
...
footnote(default_kable(outtab,
  ...
  col.names = c("QI", rep(c("Incidence No/Yes", "Crude HR (95% CI), p-value", "Adjusted HR (95% CI), p-value"), 2))
) %>%
